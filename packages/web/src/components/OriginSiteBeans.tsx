...
import { beanCatalogHref, beansByOriginHref, getBeansForOriginSlug } from "@/lib/originBeans";

const PREVIEW_LIMIT = 12;

interface OriginSiteBeansProps {
  slug: string;
  originName: string;
}
...
export function OriginSiteBeans({ slug, originName }: OriginSiteBeansProps) {
  const beans = getBeansForOriginSlug(slug);
  const preview = beans.slice(0, PREVIEW_LIMIT);

  if (beans.length === 0) {
    return (
      <section className="section-card origin-site-beans">
        <p className="section-title">DripLab で取り扱う{originName}産の豆</p>
        <p className="method-description">
          現在、産地表記に「{originName}」が含まれる豆は掲載されていません。{" "}
  ...
  return (
    <section className="section-card origin-site-beans">
      ...
      <p className="origin-site-beans-lead">
        豆一覧 {beans.length}件中、産地表記に「{originName}」が含まれるものです。
      </p>
